...
/// Handler called on an arbitrary background thread by the SDK Core when each ad request is complete. If a request failed, adSession will be nil and the errorInfo will identify the reason. Called repeatedly until `completed` is YES indicating no more callbacks will occur for the initial ad request.
...
@property (readonly, getter=isApplicationInBackground) BOOL applicationInBackground;
...
/**
 ...
 All VASAdSessions loaded during the request are returned in the completion handler, which is invoked on an
 arbitrary queue. Therefore ensure thread safety within the completion block code, typically by dispatching to your own queue. Note that a nil array of VASAdSessions may occur, in which case the completion handler's VASErrorInfo parameter
 will be set.
