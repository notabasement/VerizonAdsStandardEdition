...
#import <VerizonAdsVerizonNativeController/VASNativeComponent.h>
...
extern NSString * const kVASNativeComponentDelegateRefKey;  // VASWeakRef<id<VASVerizonNativeComponentDelegate>>

/// Deprecated, use kVASNativeComponentDelegateRefKey;
...
/**
 If there are any rules associated with the component, call this method to create them.
 
 @param view    UIView associated with the component for which rules will be created.
 */
- (void)createRulesWithView:(UIView *)view;
...
/**
Mark the impression for the specified event id as having been fired.

@param eventId   The event id used to fire the impression.
@return YES if the impression had already been marked fired, or NO for the first time.
*/
- (BOOL)markFired:(NSString *)eventId;

/**
 Used to inform the parent components of the impression event and allow them to respond.
 */
